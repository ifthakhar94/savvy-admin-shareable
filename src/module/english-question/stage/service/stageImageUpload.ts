...
export const dataSymbolConfig = (type: string) => {
  ...
  var datasymbol = JSON.stringify({
    query: `mutation S3_BUCKET_FILE_UPLOAD_CRIDENTIAL{
      stageImageData: createS3PresignedUrl(input:{
        type: stage
...
export async function stageImageUpload(imageData: any, type: string) {
  const config_bg: any = axiosConfig(dataSymbolConfig(type));
  let firstResponse;
  let imageUrl;
  await axios(config_bg)
    .then(function (response) {
      firstResponse = response?.data?.data?.stageImageData?.stageImageData;
  ...
  if (firstResponse != null) {
    firstConfig = getBgConfig(firstResponse, imageData, type);
