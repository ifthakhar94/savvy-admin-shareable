...
export const dataBgConfig = (type: string) => {
  ...
  var databg = JSON.stringify({
    ...
    specialproductquestion: createS3PresignedUrl(input:{
        type: specialproductquestion
        fileExtension: ${imageType}
    }){
        specialProductImageData{
            url
            bucket
            key
            XAmzAlgorithm
...
export async function imageUploadSpecialProduct(
  backgroundImage: any,
  type: string
) {
  ...
  await axios(config_bg)
    .then(function (response) {
      firstResponse =
        response?.data?.data?.specialproductquestion?.specialProductImageData;
