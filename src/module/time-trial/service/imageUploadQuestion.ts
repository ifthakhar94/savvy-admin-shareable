...
export const dataSymbolConfig = (type: string) => {
  ...
  var datasymbol = JSON.stringify({
    ...
    monthlyquestion: createS3PresignedUrl(input:{
...
export async function imageUploadQuestion(backgroundImage: any, type: string) {
  const config_bg: any = axiosConfig(dataSymbolConfig(type));
  let firstResponse;
  let imageUrl;
  await axios(config_bg)
    .then(function (response) {
      firstResponse = response?.data?.data?.monthlyquestion?.monthlyImageData;
